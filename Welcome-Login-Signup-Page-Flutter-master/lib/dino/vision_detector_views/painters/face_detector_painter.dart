        ...
        if (faceContour?.points != null) {
          for (final Point point in faceContour!.points) {
            if (type == FaceContourType.noseBottom) {
              ...
              if (changer.firstFrameDino) {
                changer.DinoNosePoint = point.y as int;
                changer.firstFrameDino = false;
                changer.notify();
              }

              if (!changer.firstFrameDino) {
                print("Average nose is:${changer.DinoNosePoint}");
                print("Current nose is:${point.y}");
              }

              if (!changer.firstFrameDino) {
                if (changer.DinoNosePoint.abs() - point.y.abs() > 40) {
                  print("Head up");
                  changer.isDinoHeadUp = true;
